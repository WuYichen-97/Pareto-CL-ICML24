...
class PC_CNN(MammothBackbone):
    def __init__(self, input_size: int, output_size:int)-> None:
        ...
        self.classifier1 = nn.Linear(320, self.output_size)
    ...
    def forward(self, x: torch.Tensor, returnt='out', mode='balance') -> torch.Tensor:
        """
        Compute a forward pass.
        :param x: input tensor (batch_size, input_size)
        :return: output tensor (output_size)
...
class PC_CNN(MammothBackbone):
    ...
    def __init__(self, input_size: int, output_size:int, eman =False, momentum=0)-> None:
        ...
        if eman:
            ...
            self.ema = None
        # print('self.named_parameters', self.main.net.named_parameters)
        # print('self.named_parameters1', self.ema.net.named_parameters)
        # print('self.named_parameters2', self.named_parameters)
    ...
    def get_fast_weight(self, mode='main') -> OrderedDict:
        if mode == 'main':
            ...
            # print('0',self.main.get_fast_weight)
            # print('self.main.named_params', self.main.named_parameters())
            # return OrderedDict([[p[0], p[1].clone()] for p in self.main.named_parameters()])
            # return self.main.get_fast_weight
        elif mode == 'ema':
            # return OrderedDict([[p[0], p[1].clone()] for p in self.ema.named_parameters()])
            state_dict_main = self.main.state_dict()
            state_dict_ema = self.ema.state_dict()
            for (k_main, v_main), (k_ema, v_ema) in zip(state_dict_main.items(), state_dict_ema.items()):
                assert k_main == k_ema, "state_dict names are different!"
